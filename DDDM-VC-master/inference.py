...
def inference(a): 
    ...
    # Load model
    f0_quantizer = Quantizer(hps).to(device)
    utils.load_checkpoint(a.ckpt_f0_vqvae, f0_quantizer, device)
    f0_quantizer.eval()
    ...
    utils.load_checkpoint(a.ckpt_model, model, device)
    model.eval()

...
class AMPBlock(torch.nn.Module):
    def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5), rank=0):
        ...
        self.alpha1 = nn.Parameter(torch.ones(1, channels, 1).to(rank))
        self.alpha2 = nn.Parameter(torch.ones(1, channels, 1).to(rank))
    ...
    def forward(self, x, x_mask=None):
        for c1, c2 in zip(self.convs1, self.convs2):
            ...
            xt = x + (1 / self.alpha1) * (torch.sin(self.alpha1 * x) ** 2)  # Snake1D

            if x_mask is not None:
                xt = xt * x_mask
            xt = c1(xt)

            xt = x + (1 / self.alpha2) * (torch.sin(self.alpha2 * xt) ** 2)  # Snake1D
...
class AMPBlock_filter(torch.nn.Module):
    def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5), rank=0, orig_freq=None, rolloff=0.25):
        super(AMPBlock_filter, self).__init__()

        # Assuming T.Resample is a valid class for resampling and has been imported correctly
        self.upsampling_with_lfilter = T.Resample(
            orig_freq=orig_freq, 
        ...
        self.alpha1 = [nn.Parameter(torch.ones(1, channels, 1)) for _ in range(len(self.convs1))]
        self.alpha2 = [nn.Parameter(torch.ones(1, channels, 1)) for _ in range(len(self.convs2))]

    def forward(self, x, x_mask=None):
        for c1, c2, a1, a2 in zip(self.convs1, self.convs2, self.alpha1, self.alpha2):
            xt = self.upsampling_with_lfilter(x)
            ...
            xt = self.upsampling_with_lfilter(xt)

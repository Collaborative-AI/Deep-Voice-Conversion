...
def main(args):
    ...
    os.environ['WORLD_SIZE'] = str(n_cpus)  # Update for CPU count
    os.environ['RANK'] = '0'  # CPU does not have a rank like GPU

    if n_gpus > 0:
        mp.spawn(run, nprocs=n_gpus, args=(n_gpus, args))
    else:
        # Run on CPU without multiprocessing
        run(0, n_cpus, args)


def run(rank, n_procs, args):
    if n_procs > 0:
        # Initialize the process group for CPU
        dist.init_process_group(backend='gloo', init_method='env://', world_size=n_procs, rank=rank)

    torch.manual_seed(1234)

    # Assuming DLoader is a custom DataLoader, update it to work with CPU if necessary
    dset = DLoader(args.input_dir)

    # If using CPU, you might not need a distributed sampler
    d_sampler = torch.utils.data.distributed.DistributedSampler(
        dset,
        num_replicas=n_procs,
        rank=rank,
        shuffle=True) if n_procs > 1 else None

    collate_fn = Collate()
    d_loader = DataLoader(dset, num_workers=8, shuffle=(d_sampler is None),
                          batch_size=1, pin_memory=False,  # Set to False for CPU

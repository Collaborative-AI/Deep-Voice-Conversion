...
try:
    # Load the checkpoint and move model and optimizer to CPU
    model, optimizer = utils.load_checkpoint(
        utils.latest_checkpoint_path(hps.model_dir, "G_*.pth"),
        model.to(device),  # Move model to CPU
        optimizer.to(device) if optimizer else None  # Move optimizer to CPU if provided
    )
    epoch_str = model.state_dict().get('epoch', 1)  # Assuming the checkpoint stores the epoch number
    global_step = (epoch_str - 1) * len(train_loader)
except:
    epoch_str = 1
    global_step = 0

# Move optimizer to CPU if it wasn't moved in the load_checkpoint function
if optimizer:
    optimizer.to(device)

# Initialize the scheduler and scaler with the optimizer
scheduler_g = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=hps.train.lr_decay, last_epoch=epoch_str - 2)
scaler = torch.cuda.amp.GradScaler(enabled=hps.train.fp16_run)  # Assuming you want to keep mixed-precision training

# The rest of your training loop
for epoch in range(epoch_str, hps.train.epochs + 1):
    # Since we're using CPU, the rank is not needed
    # train_and_evaluate function should be updated to handle CPU training
    train_and_evaluate(0, epoch, hps, [model, mel_fn, w2v, f0_quantizer, aug, net_v], optimizer,
                       scheduler_g, scaler, [train_loader, eval_loader if rank == 0 else None], logger, [writer, writer_eval] if rank == 0 else None, n_gpus)
    scheduler_g.step()

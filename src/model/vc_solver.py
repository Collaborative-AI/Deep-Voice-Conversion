...
class Solver(object):
    def __init__(self, config, args):
        ...
        # Build model and save configuration
        self.build_model()
        self.save_config()
    ...
    def time_shuffle(self, data):
        """Shuffle segments of the input data tensor along the specified dimension."""
        print(f"data shape: {data.shape}")
        # Ensure data has at least 3 dimensions
        if data.dim() < 3:
            raise ValueError(f"Expected 3D tensor, but got {data.dim()}D tensor with shape {data.shape}")
        
        # Split data into segments, shuffle, and concatenate back
    ...
    def load_model(self):
        """Load model and optimizer state from specified path."""
        print(f"[MAIN-VC] Loading model from {self.args.load_model_path}")
    ...
    def build_model(self):
        ...
        print("[MAIN-VC] Total parameter count: {}".format(
        ...
        # Initialize mutual information networks
        mine_size = self.config["CMI"]["mine"]
        club_size = self.config["CMI"]["club"]
        self.mi_club = CLUBSample_group(club_size, club_size, club_size).to("cuda" if torch.cuda.is_available() else "cpu")
        self.mi_mine = MINE(mine_size // 2, mine_size // 2, mine_size).to("cuda" if torch.cuda.is_available() else "cpu")
    ...
    def train(self, n_iterations):
        ...
        for iteration in range(n_iterations):
            # Adjust lambda values based on the iteration
            ...
            if data is None or len(data) == 0:
                print(f"[MAIN-VC] Warning: Received empty data at iteration {iteration}. Skipping...")
                continue

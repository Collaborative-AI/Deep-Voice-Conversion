...
class CMUArcticCorpus(Dataset):
    ...
    def __getitem__(self, idx):
        """
        Select target audio and reference audio from the CMU Arctic dataset.
        ...
        - Convert audio and reference audio to mel-spectrograms.
        """
        # Get target information from the dataset
        row = self.sentences[idx]
        og_audio, sr = librosa.load(row.path, sr=cfg['sample_rate'])
        og_audio = torch.from_numpy(og_audio)
        speaker_id = row.meta['speaker']  # Fetch the speaker ID from metadata
        
        # Clip target audio (random start position) to MAX_WAVE_LEN
        ...
        # Convert target audio to mel-spectrogram
        ...
        ref_mel_audio = librosa.feature.melspectrogram(
            y=ref_audio.numpy(),
            sr=cfg['sample_rate'],
            n_fft=cfg['filter_length'],
            hop_length=cfg["hop_length"],
        ...
        mel_audio = torch.from_numpy(mel_audio).unsqueeze(0)
        ref_mel_audio = torch.from_numpy(ref_mel_audio)

        # Prepare the final output sample
        sample = {
            'data': mel_audio,
            'wav': audio,
